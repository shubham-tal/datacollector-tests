...
@pytest.mark.parametrize('data_format', ['DELIMITED'])
@pytest.mark.parametrize('delimiter_format_type', ['CSV', 'CUSTOM', 'POSTGRES_CSV', 'TDF', 'RFC4180', 'EXCEL']) # 'MYSQL' -> Need to check its working with field having new line character, 'POSTGRES_TEXT' -> Done get
@pytest.mark.parametrize('root_field_type', ['LIST_MAP'])
@pytest.mark.parametrize('header_line', ['WITH_HEADER'])
def test_directory_origin_configuration_delimiter_format_type(sdc_builder, sdc_executor, data_format, delimiter_format_type,
                                                              delimited_file_writer, shell_executor, root_field_type, header_line):
    """Test for Delimiter format type. Here we will be creating delimited files in different formats. e.g. POSTGRES_CSV, TDF, RFC4180, etc.,
    ...
    delimiter_character = delimiter_character_map[delimiter_format_type] if delimiter_format_type in delimiter_character_map else None

    try:
        logger.debug('Creating files directory %s ...', files_directory)
        shell_executor(f'mkdir {files_directory}')
        delimited_file_writer(os.path.join(files_directory, FILE_NAME), FILE_CONTENTS, delimiter_format_type, delimiter_character)
...
def test_directory_origin_configuration_max_line_length_for_log_data_format(sdc_builder, sdc_executor, data_format,
                                                        shell_executor, file_writer, max_line_length):
